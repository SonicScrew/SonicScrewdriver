...
{
    // adjusts target for deviating from 30 day minimum maturity
    // somewhat empirical, but on the order of the deviation
    unsigned int nTargetMultiplier = 20;
    ...
    // v0.3 protocol kernel hash weight starts from 0 at the 1-day min age
    // this change increases active coins participating the hash and helps
    // to secure the network when proof-of-stake difficulty is low

    // start the hash weight at 0 from time 0, not at minimum maturity as in PPC
    int64 delta = (int64)nTimeTx - txPrev.nTime;
    if (delta < nStakeMinAge) {
        nTimeWeight = 0;
    } else {
        nTimeWeight = min(delta, (int64)nStakeCapAge);
    }
