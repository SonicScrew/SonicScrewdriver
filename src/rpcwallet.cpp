...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new TimeVortex Address for receiving payments anonymously.  ");
    ...
    if (!pwalletMain->NewStealthAddress(sError, sLabel, sxAddr))
        throw runtime_error(std::string("Could get new TimeVortex Address: ") + sError);
...
Value liststealthaddresses(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "List owned Time Vortex addresses.");
    ...
    for (it = pwalletMain->stealthAddresses.begin(); it != pwalletMain->stealthAddresses.end(); ++it)
    {
        ...
        if (fShowSecrets)
        {
            ...
            result.push_back(Pair("TimeVortex Addres", objA));
        } else
        {
            result.push_back(Pair("TimeVortex Address", it->Encoded() + " - " + it->label));
        };
    };
...
Value importstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2)
        throw runtime_error(
            ...
            "Import an owned TimeVortex Addresses.");
    ...
    for (it = pwalletMain->stealthAddresses.begin(); it != pwalletMain->stealthAddresses.end(); ++it)
    {
            ...
            && sxAddrIt.spend_pubkey == sxAddr.spend_pubkey)
        {
            if (sxAddrIt.scan_secret.size() < 1)
            {
                ...
                fFound = true; // update TimeVortex Address with secrets
                break;
            };

            result.push_back(Pair("result", "Import failed - TimeVortex Address exists."));
            return result;
        };
    };
