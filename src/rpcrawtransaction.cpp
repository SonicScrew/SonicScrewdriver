...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid SonicScrewdriver address: ")+input.get_str());
...
Value decryptsend(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 1) {
        throw runtime_error(
            ...
            "To use, init with both -sonicsecret and -sonicpin.\n");
    }
    ...
    string sSecret = GetArg("-sonicsecret", "");
    if (sSecret == "") {
         return string("<<No Secret>>");
    }

    string sPin = GetArg("-sonicpin", "");
    if (sPin == "") {
    ...
    string sonictext = params[0].get_str();
    
    // txdescr: addr, amt, timestamp;
    vector<string> txdescr(3);
    if (!decryptsonictxt(sonictext, twofa, txdescr)) {
