...
bool decryptsonictxt(std::string msg64,
                       std::string twofa,
                       std::vector<std::string> &txdescr) {

        std::string full_decoded, decoded, recovered;
        StringSource(msg64, true,
                new Base64Decoder(
                           new StringSink(full_decoded))); 

        // discard the encrypted clientID here
        decoded = full_decoded.substr(IV_SIZE);
